...
class ProjectDevelopmentSubmission(models.Model):
    ...
    submitted_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='development_submissions')
    ...
    reviewed_by = models.ForeignKey(User, null=True, blank=True, on_delete=models.SET_NULL, related_name='reviewed_development_submissions')
...
class InterimEvaluation(models.Model):
    ...
    evaluator = models.ForeignKey(User, on_delete=models.CASCADE, related_name='interim_evaluations')
...
class ProjectStageSubmission(models.Model):
    ...
    reviewed_at = models.DateTimeField(null=True, blank=True)
    reviewed_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='reviewed_stage_submissions')
    ...
    def __str__(self):
        return f"{self.project.title[:30]} - {self.get_stage_display()}"
...
class ProjectStageSubmissionVersion(models.Model):
    ...
    submitted_at = models.DateTimeField(auto_now_add=True)
    reviewed_at = models.DateTimeField(null=True, blank=True)
    reviewed_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='reviewed_stage_submission_versions')
    ...
    def __str__(self):
        return f"{self.stage_submission.project.title[:30]} - {self.stage_submission.stage} v{self.version}"
